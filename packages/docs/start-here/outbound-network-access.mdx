...
| `auth.openai.com` | Opt-in | ChatGPT Pro/Plus browser authorization and token refresh performed by the local OpenCode subprocess. | Users cannot connect or refresh subscription-backed OpenAI access. |
| `chatgpt.com` | Opt-in | Authenticated Codex response requests after ChatGPT Pro/Plus sign-in. | Subscription-backed OpenAI models cannot answer tasks. |
...
| --- | --- | --- |
| OpenAI ChatGPT subscription | `auth.openai.com`, `chatgpt.com` | Browser sign-in, token refresh, or subscription-backed Codex requests fail. |
