...
Organization install links are an upstream cloud capability and are disabled in
the AgencyAI local MVP. This fork does not ship Den or its server-side install
link implementation. The remainder of this page is retained temporarily as
upstream-profile reference material.
