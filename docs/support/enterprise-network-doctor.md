...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -Command "`$p=Join-Path `$env:TEMP 'openwork-doctor.ps1'; Invoke-WebRequest -UseBasicParsing 'https://raw.githubusercontent.com/artreimus/AgencyAI/dev/scripts/support/openwork-doctor.ps1' -OutFile `$p; & `$p -WebUrl 'https://agencyai.example.com' -ApiUrl 'https://api.agencyai.example.com' -ExpectedIssuerMatch 'DigiCert'"
```
