...
function genericSubmissionIssue(input?: {
  ...
  return {
    ...
    message: input?.message ?? "AgencyAI could not verify connected service tools for the selected model.",
  };
}
...
function authResolutionIssue(input?: { timedOut?: boolean }): CloudMcpSubmissionIssue {
  return genericSubmissionIssue({
    ...
    message: input?.timedOut
      ? "AgencyAI timed out while restoring connected service access."
      : "AgencyAI could not finish restoring connected service access.",
  ...
  if (
    ...
    return {
      ...
      issue: genericSubmissionIssue({
        ...
        message: "AgencyAI Cloud did not prove that search_capabilities and execute_capability are available.",
      }),
    };
  }
  ...
  if (!projection.checked || projection.source !== "experimental_tool") {
    return {
      ...
      issue: genericSubmissionIssue({
        ...
        recommendedAction: "Update or restart AgencyAI, then Retry. Open Connect for detailed diagnostics.",
      }),
    };
  }
  if (
...
function timeoutIssue(): CloudMcpSubmissionIssue {
  return genericSubmissionIssue({
    ...
    message: "AgencyAI timed out while preparing connected service tools.",
  });
}
  ...
  return {
    ...
    issue: timedOut
      ...
      : genericSubmissionIssue({
          ...
          message: "AgencyAI could not check connected service tools before sending.",
        }),
  };
}
