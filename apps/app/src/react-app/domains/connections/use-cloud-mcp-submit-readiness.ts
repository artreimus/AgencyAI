...
function missingContextIssue(input: {
  ...
  if (!input.client || !input.workspaceId) {
    return {
      ...
      retryable: true,
      message: "AgencyAI could not resolve the workspace service before checking connected service tools.",
  ...
  return {
    ...
    stage: "provider_projection",
    retryable: false,
    message: "AgencyAI could not verify connected service tools for this submission.",
