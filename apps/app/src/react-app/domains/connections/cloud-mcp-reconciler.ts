...
export function cloudMcpFailureStageLabel(input: {
  ...
  if (code.includes("tool_ids") || code.includes("client_registration")) return "AgencyAI components need updating";
...
export function cloudMcpRecommendedAction(input: {
  ...
  if (!input.signedIn) return "Sign in to AgencyAI Cloud.";
  ...
  if (code.includes("scope")) return "Reconnect AgencyAI Cloud with the required permissions.";
  ...
  if (isProviderProjectionFailure(input.health?.firstFailure)) return "Choose a model that can use AgencyAI Cloud tools.";
  if (code.includes("tool_ids") || code.includes("client_registration")) return "Update AgencyAI, then retry.";
  if (code === "extensions_plugin_missing") return "Reload the agent so AgencyAI instructions are current.";
  if (code === "cloud_tools_missing") return "Reconnect AgencyAI Cloud so the endpoint exposes search_capabilities and execute_capability.";
