...
function remoteSupportMessage(message: string) {
  return `${message} Upgrade the AgencyAI host and try again. If this continues, open an issue from About & Licenses.`;
}
...
export function resolveRemoteWorkspaceConnectionTarget(workspace: WorkspaceInfo): TargetResult {
  ...
  if (workspace.remoteType && workspace.remoteType !== "openwork") {
    return {
      ...
      state: {
        status: "error",
        message: "Connection diagnostics are only available for AgencyAI remote workers.",
        checkedAt: Date.now(),
      },
    };
  }
...
export async function testRemoteWorkspaceConnection(
  ...
  if (!target.token) {
    return fail(
      remoteSupportMessage(`Token is missing for ${target.endpointLabel}. Edit connection and paste a valid AgencyAI token.`),
      checkedAt,
    );
  }
