  ...
  const executeCommand = useCallback(async (
    ...
    if (request.expectedRevision !== undefined && request.expectedRevision !== revision) {
      return {
        ok: false,
        id: request.id,
        error: `AgencyAI context changed from revision ${request.expectedRevision} to ${revision}.`,
        code: "conflict",
        revision,
      };
    }
...
export function OpenworkRouteControlActions() {
  ...
  const actions = useMemo<OpenworkControlAction[]>(() => [
    ...
    },
    {
      ...
      description: "List the main capabilities of AgencyAI.",
      kind: "query",
      ...
      execute: () => ({
        capabilities: [
          ...
          { id: "voice", label: "Voice mode", description: "Talk to AgencyAI with real-time voice using OpenAI Realtime." },
          ...
          { id: "sharing", label: "Share sessions", description: "Share workspace sessions with collaborators via AgencyAI Cloud." },
        ],
