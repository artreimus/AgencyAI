...
describe("chat tool error attribution", () => {
  test("identifies an AgencyAI-created capability deadline", () => {
    expect(attributeChatToolError("The capability call exceeded 180s. Retry once.")).toEqual({
      label: "AgencyAI timeout",
      confidence: "Confirmed",
      description: "AgencyAI created this deadline. The external operation may still have completed, so verify its state before retrying.",
    })
  })

  test("identifies a structured AgencyAI lifecycle deadline", () => {
  ...
  test("identifies an AgencyAI block before send", () => {
    expect(attributeChatToolError(JSON.stringify({
      diagnostic: { code: "MCP_URL_BLOCKED", category: "security_blocked" },
    }))).toMatchObject({
      label: "Blocked by AgencyAI",
      confidence: "Confirmed",
    })
  })
